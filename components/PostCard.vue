<template>
    <div class="post-card">
        ...
        <div class="post-card__buttons">
            <NuxtLink :to="`post/${post?.id}`" style="all: unset;"><Button label="Read" width="70px"/></NuxtLink>
            <!-- <Button v-show="session?.user" label="Delete" @custom-function="deletePost(post?.id)" width="70px" /> -->
        </div>
    </div>
</template>
