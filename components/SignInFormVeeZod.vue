<template>
    <form class="signin-form" @submit.prevent="onSubmit">
        ...
        <div class="signin-form__input-card">
            <input type="text" placeholder="Email" v-model="email" :disabled="status === 'loading'">
        ...
        <Button label="Log in" width="80%" v-else/>
...
<script setup lang="ts">
    ...
    const { data: session, signIn, status } = useAuth()
    const authResponse = ref()
    const isModalOpen = ref(false)
    const fetchMessage = ref()

    onMounted(() => {
        session.value?.user && navigateTo('/')
    })
    ...
    const onSubmit = handleSubmit(async (values) => {
        await signIn('credentials', {email: values.email, password: values.password, redirect: false}).then(res => authResponse.value = res)
        if(!authResponse.value.error) fetchMessage.value = 'Logged in successfully'
        if(authResponse.value.error) fetchMessage.value = authResponse.value.error
